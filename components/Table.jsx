const Table = ({ planets }) => {
  ...
  return (
    ...
    <>
    {
      planets.length ? (
        <div>
        <table border={1} style={{marginLeft:"auto",marginRight:"auto", width:"80%",}}> 
          <thead>
          ...
          <tbody>
            {planets?.map((planet, index) => {
              return <tr key={index} style={{  color: "white"}}>
                ...
                <td style={{ margin: "0 10px" }}>{planet?.discoverymethod}</td>
                <td style={{ margin: "0 10px" }}>{planet?.disc_year}</td>
                <td style={{ margin: "0 10px" }}>{planet?.disc_facility}</td>
              </tr>
            }) }
          </tbody>
        </table>
      ...
      (<div style={{color:"white", marginLeft:"290px", fontSize:"x-large"}}>No data to show</div>)
    }
    </>
  )
}
